...
public class GameMenu extends Application {
    ...
    private void starter() {
        ...
        building = new Building(null, BuildingType.PERIMETER_TOWER, 31,20);
        for(int i = 19; i < 22; i++) for(int j = 30; j < 33; j++) map[i][j].setBuilding(building);
        building = new Building(null, BuildingType.DEFENSE_TURRET, 34,20);
        for(int i = 19; i < 22; i++) for(int j = 33; j < 36; j++) map[i][j].setBuilding(building);
        building = new Building(null, BuildingType.SQUARE_TOWER, 37,20);
        for(int i = 19; i < 22; i++) for(int j = 36; j < 39; j++) map[i][j].setBuilding(building);
        building = new Building(null, BuildingType.ROUND_TOWER, 40,20);
        for(int i = 19; i < 22; i++) for(int j = 39; j < 42; j++) map[i][j].setBuilding(building);
